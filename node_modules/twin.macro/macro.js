...
var configTwinDefaults = function (state) { return (Object.assign({}, {allowStyleProp: false,
...
var getConfigTailwindProperties = function (state, config) {
  ...
  var configTailwind = configExists ? resolveTailwindConfig([require(configPath), defaultTailwindConfig]) : resolveTailwindConfig([defaultTailwindConfig]);
...
var getConfigTwin = function (config, state) { return (Object.assign({}, configTwinDefaults(state),
  config)); };
...
var getKeyframesString = function (keyframes) { return Object.entries(keyframes).map(function (ref) {
  var name = ref[0];
  var frames = ref[1];
...
var precheckGroup = function (ref) {
  ...
  return throwIf(classNameRaw === 'group', function () { return ("\"group\" must be added as className:\n\n" + (logBadGood('tw`group`', '<div className="group">')) + "\n"); });
};
...
var getStyles = (function (classes, t, state) {
  throwIf([null, 'null', undefined].includes(classes), function () { return logGeneralError('Only plain strings can be used with "tw".\nRead more at https://twinredirect.page.link/template-literals'); }); // Strip pipe dividers " | "
...
var handleTwProperty = function (ref) {
  ...
  if (cssAttributes.length > 0) {
    ...
    if (expr.isArrayExpression()) {
      ...
      throwIf(!expr.node, function () { return logGeneralError("An empty css prop (css=\"\") isn’t supported alongside the tw prop (tw=\"...\")"); });
